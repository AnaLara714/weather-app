...
export default function Main() {
  ...
  return (
    <div className=
      {`${ itFound === false ? (isLoading === true ? 
        <button className="bg-[#f1f5f9] w-32 h-16 p-4 flex justify-center rounded" disabled>
          <svg className="animate-[spin_8000s_linear_infinite] flex flex-row  justify-center items-center"/>Buscando...
        </button> : "max-sm:h-screen bg-[#f1f5f9]" ) : infosCity?.current.is_day === 1 ? "bg-[#fbbf24]" : "bg-[#155e75]"}  
